...
func (m *GoCode) MarshalToSizedBufferVT(dAtA []byte) (int, error) {
	...
	if len(m.QueryStructName) > 0 {
		i -= len(m.QueryStructName)
		copy(dAtA[i:], m.QueryStructName)
		i = encodeVarint(dAtA, i, uint64(len(m.QueryStructName)))
		i--
		dAtA[i] = 0x1
		i--
		dAtA[i] = 0xaa
...
func (m *GoCode) UnmarshalVT(dAtA []byte) error {
	...
	for iNdEx < l {
		...
		case 21:
			if wireType != 2 {
				return fmt.Errorf("proto: wrong wireType = %d for field QueryStructName", wireType)
			}
			var stringLen uint64
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
			...
			m.QueryStructName = string(dAtA[iNdEx:postIndex])
			iNdEx = postIndex
		default:
			iNdEx = preIndex
			skippy, err := skip(dAtA[iNdEx:])
